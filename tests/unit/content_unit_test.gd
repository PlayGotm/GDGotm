...
func test_all() -> UnitTestInfo:
	...
	for value in ["herpderp", {"herp": "derp"}, ["herp", "derp"]]:
		...
		Utility.assert_equality(JSON.stringify(value), JSON.stringify(loaded_value))
	...
	Utility.assert_equality(loaded_node.position.x, 1337)
	Utility.assert_equality(loaded_node is Control, true)
	...
	Utility.assert_equality(bytes_to_var(fetched_data), string_data)

	Utility.assert_equality(content.size, fetched_data.size())


	# Get by key
	Utility.assert_resource_equality(await GotmContent.get_by_key(key), content)

	# Get contents by directory
	var directory_contents: Array = await GotmContent.list(GotmQuery.new().filter("directory", directory))
	Utility.assert_equality(directory_contents.size(), 1)
	Utility.assert_resource_equality(directory_contents[0], content)
	...
	Utility.assert_resource_equality(updated_content, content)
	var new_fetched_data: PackedByteArray = await GotmContent.get_data(updated_content)
	Utility.assert_equality(bytes_to_var(new_fetched_data), new_string_data)

	# Delete by key
	await GotmContent.delete_by_key(key)
	Utility.assert_equality(await GotmContent.get_by_key(key), null)

	# Create local 
	var local_content: GotmContent = await GotmContent.create_local(var_to_bytes(string_data), key)
	Utility.assert_resource_equality(await GotmContent.fetch(local_content), local_content)
	Utility.assert_resource_equality(await GotmContent.get_by_key(key), local_content)
	...
	Utility.assert_resource_equality(top_level_hard, [content3, content1])
	...
	Utility.assert_resource_equality(more_than_level_one, [content3, content2])
	...
	Utility.assert_resource_equality(best_map_search, [named_content])
	...
	Utility.assert_resource_equality(my_private_contents, [private_content])
	Utility.assert_resource_equality(non_private_contents, [content3, content2, content1])
	...
	if auth.is_registered:
		...
		Utility.assert_resource_equality(top_upvoted_contents, [content2, content3, content1])
	Utility.assert_resource_equality(await GotmMark.list_by_target(content2), [upvote])
	Utility.assert_resource_equality(await GotmMark.list_by_target(content1), [downvote])
	...
	Utility.assert_equality(upvote_count, 1)
	Utility.assert_equality(downvote_count, 1)
	await GotmMark.delete(upvote)
	await GotmMark.delete(downvote)
	Utility.assert_equality(await GotmMark.get_count_with_type(content2, GotmMark.Types.UPVOTE), 0)
	Utility.assert_equality(await GotmMark.get_count_with_type(content1, GotmMark.Types.DOWNVOTE), 0)
	...
	Utility.assert_resource_equality(child.parent_ids, [parent1.id, parent2.id])
	
	var children: Array = await GotmContent.list(GotmQuery.new().filter("parent_ids", [parent1]))
	Utility.assert_resource_equality(children, [child])
	...
	Utility.assert_resource_equality(parents,  [content1, content2, content3, parent1, parent2])
	
	
	await GotmContent.delete(parent1)
	
	child = await GotmContent.fetch(child)
	Utility.assert_resource_equality(child.parent_ids, [parent2.id])
	
	children = await GotmContent.list(GotmQuery.new().filter("parent_ids", [parent1]))
	Utility.assert_resource_equality(children, [])
	children = await GotmContent.list(GotmQuery.new().filter("parent_ids", [parent2]))
	Utility.assert_resource_equality(children, [child])
	children = await GotmContent.list(GotmQuery.new().filter("parent_ids", [parent1, parent2]))
	Utility.assert_resource_equality(children, [])
	...
	Utility.assert_resource_equality(parents,  [content1, content2, content3, parent2])
	
	await GotmContent.delete(parent2)
	
	child = await GotmContent.fetch(child)
	Utility.assert_equality(child, null)
	...
	parents = await GotmContent.list(GotmQuery.new().filter("parent_ids", []).sort("created", true))
	Utility.assert_resource_equality(parents,  [content1, content2, content3])
