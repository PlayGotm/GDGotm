...
static func get_implementation(id = null)->GDScript:
...
static func get_data(blob_or_id, type:String = ""):
	var id = _coerce_id(blob_or_id)
	if !id:
		yield(_GotmUtility.get_tree(), "idle_frame")
		return
	var data = yield(
					get_implementation(id).fetch(
							"%s/%s"%[_Gotm.get_global().storageApiEndpoint,id]
						),
					"completed"
				)
	if !data || !type:
...
static func _format(data, blob):##->GotmBlob:
	if !data || !blob:
		return
	blob['id'] = data.path
